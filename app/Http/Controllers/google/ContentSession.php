...
// die($path);
require_once '/var/www/html/facerekon/vendor/autoload.php';
/**
...
class ContentSession
{
  ...
  public function __construct()
  {
    ...
    if (array_key_exists('noconfig', $options)) {
      ...
      $filePath = $dir . 'content/merchant-info.json';
      // $this->dd($filePath);
      $configFile = $filePath;
  ...
  private function prepareServices($client)
  {
    ...
    if (!empty($endpoint)) {
      ...
      if (
        !array_key_exists('scheme', $endpointParts)
  ...
  private function getServiceWithEndpoint($client, $rootUrl, $basePath)
  {
    ...
    $gsscProps = array_diff(
      $gsscClass->getProperties(ReflectionProperty::IS_PUBLIC),
      $gsProps
    );
  ...
  public function retrieveConfig()
  {
    ...
    foreach ($response->getAccountIdentifiers() as $accountId) {
      if (
        !is_null($accountId->getAggregatorId()) &&
      ...
      if (
        !is_null($accountId->getMerchantId()) &&
  ...
  private function getToken(Google_Client $client)
  {
    ...
    printf(
      "Visit the following URL and log in:\n\n\t%s\n\n",
  ...
  public function retry($object, $function, $parameter, $maxAttempts = 5)
  {
    ...
    while ($attempts <= $maxAttempts) {
      try {
        ...
        printf(
          "Attempt to call %s failed, retrying in %d second(s).\n",
          $function,
          $sleepTime
        );
        sleep($sleepTime);
